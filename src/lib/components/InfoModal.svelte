	...
	const sections = $derived<InfoSection[]>(lang === 'en' ? [
		...
		},
		{
			...
			items: [
				...
				{
					title: 'Categories',
					text: 'Items are recognized automatically in German and English. Clear product names can also receive a more specific icon — for example fruit, vegetables, dairy cups, drink cartons, sauces, oils, snack bags, cereal, detergent, tea, coffee, fish, eggs, tubes, or paper goods. Sorting and colour still follow the regular category; flavour and ingredient words do not override a clear product type. Ambiguous or unknown names keep the familiar category icon. Long-press an item to choose a category — including “Everything else” — or switch back to Automatic. When you deliberately choose a category, Groly privately remembers it for your account and exactly that item name; future items with the same name use it automatically. Choosing Automatic forgets that preference. Editing only the name or quantity does not teach anything. The specific icon remains automatic. You can enable and reorder global category sorting in Settings. For per-list sorting, long-press a list name → Edit → "Category sorting".',

<script lang="ts">
	...
	let {
		sheet = $bindable<WaterEditLogSheetType | null>(null),
		onreload
	}: {
		sheet: WaterEditLogSheetType | null;
		onreload: () => void;
	} = $props();
	...
	async function save() {
		...
		try {
			...
			await fetch(`/api/water-logs/${sheet.id}`, {
				method: 'PUT',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ amountMl: sheet.amountMl, loggedAt: d.getTime() })
			});
			sheet = null;
			onreload();
		} finally {
	...
	async function del() {
		if (!sheet) return;
		const res = await fetch(`/api/water-logs/${sheet.id}`, { method: 'DELETE' });
		if (!res.ok) return;
		sheet = null;
		onreload();
	}
	...
	<div class="fixed bottom-0 left-0 right-0 z-50 max-w-[430px] mx-auto rounded-t-3xl"
		...
		<div class="p-6 space-y-5">
			...
			<p class="font-semibold text-base" style="color: var(--color-on-surface)">
				{t.supplement_log_edit_title} · {t.water_title}
			</p>
			...
			<div class="flex gap-2">
				...
				<!-- Time -->
				<div class="shrink-0">
					<label for="water-edit-time" class="text-xs font-medium mb-1.5 block" style="color: var(--color-on-surface-variant)">{t.supplement_log_time}</label>
					<input
						id="water-edit-time"
						type="time"

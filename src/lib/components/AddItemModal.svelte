<script lang="ts">
	let { item = null, onSave, onClose }: {
		item?: { name: string; quantityInfo: string | null } | null;
		onSave: (name: string, quantityInfo: string) => void;
		onClose: () => void;
	} = $props();
...
<div class="fixed bottom-0 left-0 right-0 z-50 rounded-t-3xl px-6 pb-8 pt-4"
	...
	<h2 class="text-lg font-bold mb-5" style="color: var(--color-on-surface)">{item ? 'Item bearbeiten' : 'Item hinzufügen'}</h2>
	...
	<div class="space-y-3 mb-6">
		...
		<div class="rounded-xl px-4 py-3.5" style="background-color: var(--color-surface-container)">
			<input
				type="text"
				placeholder="Menge / Info (z.B. 3 Stück, 1 Liter)"
	...
	<div class="flex gap-3">
		...
		<button
			onclick={() => name.trim() && onSave(name.trim(), quantityInfo.trim())}
			disabled={!name.trim()}
